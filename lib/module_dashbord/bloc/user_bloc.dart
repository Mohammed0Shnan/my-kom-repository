...
class UsersBloc extends Bloc<UsersEvent,UsersStates>{
  ...
  UsersBloc() : super(UsersLoadingState()) {
    ...
    on<UsersEvent>((UsersEvent event, Emitter<UsersStates> emit) {
      if (event is UsersLoadingEvent)
      {
        ...
        emit(UsersSuccessState(users: event.users,admins: event.admins,message: null));
      }
  ...
  void getsUsers() {
    ...
    _service.usersPublishSubject.listen((value) {
      ...
      if(value != null){
        this.add(UsersSuccessEvent(users:value['users']! ,admins:value['admins']!));
...
class UsersSuccessEvent  extends UsersEvent  {
  List<AppUser>  users;
  List<AppUser>  admins;
  UsersSuccessEvent({required this.users,required this.admins});
}
...
class UsersSuccessState extends UsersStates {
  ...
  String? message;
  UsersSuccessState({required this.users,required this.admins,required this.message});
}
...
class UsersDeletedErrorState extends UsersStates {
  ...
  UsersDeletedErrorState({ required this.data,required this.message});
}
